...
public class Robot_OmniDrive {
    ...
    public void initDrive(LinearOpMode opMode) {
        ...
        left_back      = myOpMode.hardwareMap.get(DcMotor.class, "left_back");
        right_back     = myOpMode.hardwareMap.get(DcMotor.class, "right_back");

        left_front.setDirection(DcMotor.Direction.REVERSE); // Positive input rotates counter clockwise
        right_front.setDirection(DcMotor.Direction.REVERSE);// Positive input rotates counter clockwise
        left_back.setDirection(DcMotor.Direction.REVERSE); // Positive input rotates counter clockwise
    ...
    public void moveRobot() {
        ...
        double back_left = driveYaw - driveAxial + (driveLateral * 0.5);
        double back_right = driveYaw + driveAxial + (driveLateral * 0.5);
        double front_left = driveYaw - driveAxial - (driveLateral * 0.5);
        double front_right = driveYaw + driveAxial - (driveLateral * 0.5);

        // normalize all motor speeds so no values exceeds 100%.
        double max = Math.max(Math.abs(back_right), Math.abs(front_right));
        max = Math.max(max, Math.abs(front_left));
        max = Math.max(max, Math.abs(back_left));
        if (max > 1.0)
        {
            back_left /= max;
            back_right /= max;
        ...
        left_back.setPower(back_left / 3);
        right_back.setPower(back_right / 3);
        left_front.setPower(front_left / 3);
        right_front.setPower(front_right / 3);

        // Display Telemetry
        myOpMode.telemetry.addData("Axes  ", "A[%+5.2f], L[%+5.2f], Y[%+5.2f]", driveAxial, driveLateral, driveYaw);
        myOpMode.telemetry.addData("Wheels", "FL[%+5.2f], FR[%+5.2f], LB[%+5.2f], RB[%+5.2f]", front_left, front_right, back_left, back_right);
    }
